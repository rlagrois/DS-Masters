...
pdf("MonteCarloexample.pdf", width = 8, height = 5)
par(mar = c(2,4,1,1))
...
d0
#Set offspring birth rate and find first offspring time
lambda = 0.5
birth1 = rexp(1, rate = lambda)
birth1
#Generate second offspring time
itime = c(birth1, rexp(1, rate = lambda))
itime
#Generate third offspring time
itime = c(itime, rexp(1, rate = lambda))
itime
#Sum offspring times
cumsum(itime)
#Generate fourth offspring time and sum
itime = c(itime, rexp(1, rate = lambda))
btime = cumsum(itime)
btime
#Set offsprings to where cumulative offspring time is less than job lifetime
btime = btime[ btime < d0 ]
#Generate completion times of all three first generation children
dtime = d0 + rexp(n = length(btime), rate = kappa) 
dtime
#Birth time of first child's child
btime[1] + rexp(1, rate = lambda)
#Completion time of first child's child
dtime[1] + rexp(1, rate = kappa)

#Function generates children while birth time is < parent lifetime end, similar to above using kappa to determine completion time of offspring
# Each call generates a set of children for a new parent
genKids = 
...
#Function generates birth times while birth time < parent lifetime end without using kappa, births are recursively added to list
...
#Function uses genBirth to generate children similar to genKids
genKidsR = 
  function(bTime, cTime, lambda = 0.5, kappa = 0.3) {
    # Parent job born at bTime and completes at cTime
...
pdf("SimulatedOffspringVSPoissonDist.pdf", width = 8, height = 5)
...
#Chi-Square goodness of fit test between generated kids and poisson(2.5)
obsCt = table(numKids)
...
#Function generates children based on average number of kids using poisson dist and their times using uniform dist.
genKidsU = 
  function(bTime, cTime, lambda = 0.5, kappa = 0.3) {
...
seedx = 116201412
set.seed(seedx)
#Compare runtimes for functions with vs without while 'loop'
time1 = system.time( replicate(4000, genKids(1, cTime = 9)) )
time2 = system.time( replicate(4000, genKidsU(1, cTime = 9)) )
time1/time2
#The genKids function (uses 'with' loop) is faster at completion time of 9
time1 = system.time( replicate(4000, genKids(1, cTime = 100)) )
time2 = system.time( replicate(4000, genKidsU(1, cTime = 100)) )
time1/time2
#The genKidsU function (uses distributions) is faster at completion time of 100

#View a table of times to complete each function call
set.seed(seedx)
...
#Function becomes vectorized to begin integrating a family tree
...
#Function modified to return data frames
...
#Function creates a family tree with a limiter on maxGen to prevent infinite runtime
...
#Function modified to cap time as opposed to generation
...
#Function modified to cap based on maximum offspring or maximum generation
...
#Generate tree using familyTreeT where maxTime = 8
set.seed(seed2)
...
# Helper function to call familyTree that returns summary statistics for analysis
...
#Function allows running of multiple parameters through parameter matrix
