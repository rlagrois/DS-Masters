...
```
  ...
  Before starting analysis the necessary libraries must be loaded in as well as the cleaned up dataframes from above.  Despite cleaning some NAs managed to slip by and are filtered out using the code directly below the loading of the dataframes.  While these small number of NAs didn't cause widespread problems, it was found they were an issue for the boxplots colored by median which will be displayed below.  
