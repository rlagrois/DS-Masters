...
String s5Text = "Around the New Year Zika is diagnosed in patients in the United States.";
String s5Text2 = "All of the cases were aquired in South America, it is unknown if it is yet in the local mosquito population.";
...
void mouseClicked(){
  ...
  if(mouseX > 460 && mouseX < 500 && mouseY > 0 && mouseY < 70){
    slide -= 1;
}
}
...
void present(){
  ...
  if(slide == 3){
    ...
    slide4();
    fill(0);
    text(s4Text, width * .05, height * .15);
    text(s4Text2, width * .05, height * .175); 
  }
  if(slide == 4){
    disYear = 2016;
    slide4();
    ...
    ellipse(846, 331, 4, 4);
    fill(0);
    text(s5Text, width * .05, height * .15);
    text(s5Text2, width * .05, height * .175);
  }
  if(slide == 5){
    slide = 0;
    disYear = 1947;
